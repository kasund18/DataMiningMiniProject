...
synonym_artist = ['ගායකයා','ගයනවා','ගායනා','ගායන','ගැයු','ගයන','ගයපු']
synonym_writer = ['ගත්කරු','රචකයා','ලියන්නා','ලියන','රචිත','ලියපු','ලියව්‌ව','රචනා','රචක','ලියන්','ලියූ']
synonym_genre = ['ඇල්බමය', 'ඇල්බම්']

synonym_list = [ synonym_artist, synonym_writer, synonym_genre ]
...
def search(search_query):
    ...
    search_fields = []
    year = 0
    field_list = ["artist", "writer","genre","music", "metaphor" ]
    all_fields = ["title","artist", "writer", "music", "lyrics", "metaphor"]
    final_fields = []
    year_array = []
